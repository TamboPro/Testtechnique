...
{
    internal class DossierContextMock
    {
        private DbConnection _connexion;
        private DbContextOptions<DossierContext> _contextOptions;
        public DossierContext Context { get; }


        public DossierContextMock()
        {

            _connexion = new SqliteConnection("Filename=:memory:");
            _connexion.Open();
            _contextOptions = new DbContextOptionsBuilder<DossierContext>().UseSqlite(_connexion).Options;

            Context = new DossierContext(_contextOptions);
